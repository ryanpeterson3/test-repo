...
export default async function handler(req, res) {
    if (req.headers.authorization) {
        ...
        try {
            const url = `https://api.hubapi.com/marketing/v3/emails?includeStats=true&limit=100`;
            ...
            while (!allPagesQueried) {
                if (after !== null) {
                    ...
                    const url = `https://api.hubapi.com/marketing/v3/emails?includeStats=true&limit=100&after=${after}`;
                    const response = await fetch(url, options);
                    const data = await response.json();
